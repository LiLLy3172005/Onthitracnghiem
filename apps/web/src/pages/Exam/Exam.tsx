...
type Difficulty = "de" | "trungbinh" | "kho";
type MediaType = "image" | "audio" | "formula";
...
type SetStatus = "draft" | "published";
...
const SUBJECTS = ["Tất cả", "Toán", "Vật lý", "Hóa học", "Tiếng Anh"];
        ...
        {filtered.map((q) => (
          <li key={q.id} className={`question-row question-row--${q.difficulty} ${q.status === "hidden" ? "question-row--hidden" : ""}`}>
            ...
            <div className="question-row__actions">
              <button className="icon-btn" title="Xem chi tiết" onClick={() => setDetailTarget(q)}>
                <IconEye />
              </button>
...
export const Exam: React.FC = () => {
  ...
  const handleDuplicateSet = (s: ExamSet) => {
                ...
                {examSets.map((s) => (
                  <li key={s.id} className="set-card">
                    ...
                    <div className="set-card__actions">
                      ...
                      <button className="icon-btn" title="Nhân bản" onClick={() => handleDuplicateSet(s)}>
                        <IconCopy />
                      </button>
                      <button className="icon-btn icon-btn--danger" title="Xóa bộ đề" onClick={() => handleDeleteSet(s.id)}>
                        <IconTrash />
                      </button>
                    </div>
                  </li>
